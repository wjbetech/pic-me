...
| Game Options / Pick a Game Mode screen | The pre-game settings screen (`src/components/GameOptions/`) — header + dropdowns per mode. Persists selection to `pic-me:mode` / `pic-me:settings` and must survive refresh without resetting. Card-forward sticker sheet that echoes homepage tokens, not photos. |
