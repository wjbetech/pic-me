...
| Theme | Light/dark via daisyUI (`src/store/themeStore` + `src/context/ThemeContext`). Must keep contrast parity between themes; primary/accent actions use the `.btn-pop` scheme — paired content tokens with candy depth (inner sheen, tonal squish edge, colored halo) instead of hard outlines. |
